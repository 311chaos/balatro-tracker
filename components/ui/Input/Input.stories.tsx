...
const STATE_COLS: {
  ...
  {
    label: 'With value',
    props: { value: 'Unstoppable', readOnly: true, 'aria-label': 'With value' },
  },
  {
    label: 'Focus',
  ...
  {
    label: 'Readonly',
    props: { value: 'Read only', readOnly: true, 'aria-label': 'Readonly' },
  },
];
    ...
    <div className="flex flex-col gap-4 p-2">
      <div className="flex items-center gap-4">
        ...
        {STATE_COLS.map(({ label }) => (
          <span
            key={label}
            className="text-muted-foreground w-36 text-center text-[10px] font-medium tracking-widest uppercase"
          >
            {label}
          </span>
        ))}
